  ...
  methods: {
    ...
    chooseDate(day, month, year) {
        ...
        if (this.mode == 1) {
            ...
            this.$emit("callback", choose);
        }else if(this.mode == 4 &&(this.startDates || this.endDates) ){
			  choose.dayCount = this.endDates ?(this.endDates - this.startDates) / 24 / 3600 / 1000 :'';
			 choose.endStr =  this.endDates ?this.dateFormat(this.endDates) : '';
			 this.$emit("callback", choose);
		}else if (this.mode == 3) {
